...
import 'package:engaz_task/app/core/isar/isar_service.dart';
import 'package:flutter/foundation.dart';
import 'package:isar/isar.dart';

/// An abstract base class for managing local
///  data persistence using Isar database
///
...
abstract base class IsarSource<D, C extends CacheModel<D>> {
  ...
  C? get(int id) =>
      isarService.db.txnSync(() => isarService.db.collection<C>().getSync(id));

  C? get first {
    return isarService.db
        .txnSync(() => isarService.db.collection<C>().where().findFirstSync());
  }

  List<T> getAll<T>() {
    return isarService.db
        .txnSync(() => isarService.db.collection<T>().where().findAllSync());
  }
  ...
  void clear() => isarService.db
      .writeTxnSync(() => isarService.db.collection<C>().clearSync());

  Stream<C?> watchObject(int id) =>
      isarService.db.collection<C>().watchObject(id);

  Stream<void> watchCollection() => isarService.db.collection<C>().watchLazy();

  Future<C?> getFirst() async {
    return isarService.db.txn(() {
      return isarService.db.collection<C>().where().findFirst();
    });
  }
}
