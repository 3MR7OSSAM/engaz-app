...
/// type-safe state transitions and preserves
...
/// - [Failure]: Represents a st
/// ate where an error occurred during data handling.
///
/// Key Features:
...
sealed class Status<T> extends Equatable {
  ...
  /// Transitions the current state to a [Failure]
  /// state with the provided exception.
  /// Preserves the existing data before the failure occurred.
  ///
  /// [exception] The [GenericException] that caused the failure.
  ///
  /// Returns a new [Failure] state with the
  ///  exception and optional previous data.
  Failure<T> toFailure(GenericException exception) => switch (this) {
        Initial<T>(initialData: final oldData) =>
          Failure(exception, oldData: oldData),
        Loading<T>(currentData: final oldData) =>
          Failure(exception, oldData: oldData),
        Success<T>(newData: final oldData) =>
          Failure(exception, oldData: oldData),
        Failure<T>(oldData: final oldData) =>
          Failure(exception, oldData: oldData),
      };
